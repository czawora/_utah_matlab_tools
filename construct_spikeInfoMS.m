...
p.addParameter('bp_fname_suffix', 'mda_chan', @ischar);
...
clip_features_fname = p.Results.clip_features_fname;
clips_raw_fname = p.Results.clips_raw_fname;
clips_spike_fname = p.Results.clips_spike_fname;
clips_whiten_fname = p.Results.clips_whiten_fname;
...
% find refset jacksheets in session_path_ls
refset_jacksheet_filt = cellfun( @(x) contains(x, 'jacksheet_refset') , session_path_ls_names);
sorted_refset_jacksheet_names = sort(session_path_ls_names(refset_jacksheet_filt));

num_refset_jacksheet = length(sorted_refset_jacksheet_names);
refset_jacksheets = cell(num_refset_jacksheet, 1);

for iJack = 1:num_refset_jacksheet
    refset_jacksheet_fpath = [session_path '/' sorted_refset_jacksheet_names{iJack}];
    refset_jacksheets{iJack} = readtable(refset_jacksheet_fpath);
end
