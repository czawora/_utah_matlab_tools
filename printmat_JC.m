function [] = printmat_JC(a,name,rlab,clab)
%PRINTMAT_JC Print matrix with labels.
%   PRINTMAT_JC(A,NAME,RLAB,CLAB) prints the matrix A with the row labels
...
%
%   PRINTMAT_JC(A,NAME) prints the matrix A with numerical row and column
...
while col<=ncols
    ...
    %s = space(ones(1,len+1-4)); %len+1-4 should equal the number in ***
    for j=0:n,
    ...
    for i=1:nrows,
        ...
        for j=0:n,
            element = a(i,col+j);
            if element==0,
                %s=[s,'     0']; % number of spaces should equal len, JC
                s=[s,space(ones(1,len)),'0'];
            elseif isinteger(element)% JC ***
                s=[s,sprintf(' %10d',element)];
            elseif (element>=1.e6)|(element<=-1.e5)|(abs(element)<.0001)
                s=[s,sprintf(' %10.3e',element)];
            else
                s=[s,sprintf(' %10.3f',element)];
            end
        end
        disp(s)
    end % for
