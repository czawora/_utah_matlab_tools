...
%        loading    = matrix  (If load_eegfile was previously ran, you can use eeg_ts as an input to skip re-loading data. The input parameters have to match those of the...
...
def_task=[];
def_hwType = 'subdural';
def_bad_chans_local_dir = fullfile(filesep,'Users','chapetonji','Desktop','JC_Extract','bad_chans_JC');
def_bad_chans=[];

%
addParameter(inp_pars,'loading',def_loading,@(x) ismatrix(x));
...
if ~isempty(BL_sess)
    ...
    alldata_BL = aux_func1(eeg_re_ref_BL,Fs,T,overlap);
...
alldata = aux_func1(eegfile_re_ref,Fs,T,overlap);
...
if visual
    ...
    if filt_ind && isempty(freq_range)
    ...
    if saving
        ...
        if any(ismember(batch_funcs,parent_funcs)) || ~disp_ind
            batch_flag=1;
            close(h)
            clear h
...
function alldata_struct = aux_func1(eegfile,Fs,T,overlap)
% calculate nwins and initialize variables

% nwins = floor((size(eegfile,1)-T*Fs)/(T*Fs-round(overlap*T*Fs)))+1;
% nfft=ceil((T*Fs+1)/2);
% 
% amps=nan(size(eegfile,2),nfft);
% tf_rep=nan(size(eegfile,2),nfft,nwins);
% freqs_FFT=nan(size(eegfile,2),nfft);
% t_pow=nan(size(eegfile,2),nwins,2);

% NN=min(sum(~isnan(eegfile),1));
% nwins = floor((NN-T*Fs)/(T*Fs-round(overlap*T*Fs)))+1;
% nfft=ceil((T*Fs+1)/2);
% 
% amps=nan(size(eegfile,2),nfft);
% tf_rep=nan(size(eegfile,2),nfft,nwins);
