...
p.addParameter('used_chan_fpath', '', @ischar);
...
used_chan_fpath = p.Results.used_chan_fpath;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%timing creation of spikeInfo

time_fid = fopen([sessRoot '/spikeInfo_time.txt'], 'w');
...
used_chan = textscan(used_chan_fid,'%s\n');
used_chan = used_chan{1};
...
refset_array_num = referencing_info{5};
refset_array_location = referencing_info{6};
...
if length(used_chan) ~= num_channels
   error('length(used_chan) ~= num_channels --- the number of split files with results does not equal the number of channels in used_chan.txt');
end
...
for i = 1:num_channels
    ...
    bp_pathf = [channel_path '/' channel_dname '.' bp_fname_suffix];
...
for iChan = 1:size(data_cmat, 1)
    ...
    current_channel_unit_names = cellstr(cellfun(@(x) [current_channel_ename '_' sprintf( '%02d', x )] , num2cell(1:current_channel_num_unit), 'UniformOutput', 0));
...
sessUniqueUnitID    = expandUnitIDInfo(spikeInfo_filt.uniqueUnitID, refset_array_num, refset_array_location);
...
function sessUniqueUnitID = expandUnitIDInfo(uniqueUnitID, refset_array_num, refset_array_location)

    if ~isnumeric(refset_array_num)
       
        refset_array_num = str2num(refset_array_num);
    end
    
    if ~isnumeric(refset_array_location)
        
       refset_array_location = str2num(refset_array_location); 
    end
