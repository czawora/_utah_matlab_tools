...
if visual
    ...
    if save_flag && ~isempty(chan_ind)
        ...
        % checks for nans
        eeg_ts_ref_row_nansum = sum(isnan(eeg_ts_ref), 2);
        eef_ts_raw_row_nansum = sum(isnan(eeg_ts_raw), 2);
        
        eeg_ts_ref_allnan_row = (eeg_ts_ref_row_nansum == size(eeg_ts_ref, 2));
        eeg_ts_raw_allnan_row = (eef_ts_raw_row_nansum == size(eeg_ts_raw, 2));
 
        % are any selected all channels all nans
        if any(eeg_ts_ref_allnan_row) || any(eeg_ts_raw_allnan_row)
        
            %find a time column that has data on each channel
            
            all_eeg_ts_ref=ds_eeg_avg_reref(:,chan_ind)';
            all_eeg_ts_raw=ds_eeg_raw_proc(:,chan_ind)';
            
            combined_ts_ref_raw = [all_eeg_ts_ref ; all_eeg_ts_raw];
            ...
            if ~any(combined_col_notallnan)
                ...
                combined_col_notallnan_idx = find(combined_col_notallnan);
                ind = combined_col_notallnan_idx(1);
                
                if ind + npoints > size(ds_eeg_avg_reref,1)
                    npoints = size(ds_eeg_avg_reref,1) - ind -1;
                end
        ...
        if all(diff(temp))
            set(h1_ax,'yaxislocation','right','ytick',temp(~isnan(temp)),'yticklabel',chan_names(chan_ind),'ylim',[min(eegspaced_raw(:))-Sig_raw(1) max(eegspaced_raw(:))+Sig_raw(end)])
        else
